-- CORRECT FIX: uploaded_by is UUID, not TEXT!
-- No type casting needed - all UUIDs
  ...
  IF TG_OP = 'INSERT' AND NEW.is_active = true THEN
    ...
    FOR v_user IN 
      ...
      WHERE np.notification_type = 'attachment_uploaded'
        ...
        AND u.user_id != NEW.uploaded_by  -- Both are UUID
        AND (
          u.user_id = v_kpi.owner_user_id  -- Both are UUID
          OR EXISTS (
...
COMMENT ON FUNCTION notify_kpi_attachment_upload() IS 'CORRECT: All fields are UUID, no casting needed';
COMMENT ON FUNCTION notify_evidence_attachment_upload() IS 'CORRECT: All fields are UUID, no casting needed';
