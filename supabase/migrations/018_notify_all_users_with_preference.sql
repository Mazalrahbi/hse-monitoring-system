-- Notify ALL users who have attachment notifications enabled
-- No role or ownership restrictions - just preference-based
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.is_active = true THEN
    ...
    SELECT * INTO v_uploader FROM app_user WHERE user_id = NEW.uploaded_by;
    
    -- Find ALL users who have this notification enabled (no role/owner restrictions)
    FOR v_user IN 
      ...
      WHERE np.notification_type = 'attachment_uploaded'
        ...
        AND u.user_id != NEW.uploaded_by  -- Don't notify the uploader themselves
    LOOP
      -- Create notification in queue for each user
...
COMMENT ON FUNCTION notify_kpi_attachment_upload() IS 'Notifies ALL users with attachment notification preference enabled (no role/ownership restrictions)';
COMMENT ON FUNCTION notify_evidence_attachment_upload() IS 'Notifies ALL users with attachment notification preference enabled (no role/ownership restrictions)';
