-- Fix reverse type mismatch: text = uuid
-- The uploaded_by field is TEXT, not UUID
  ...
  IF TG_OP = 'INSERT' AND NEW.is_active = true THEN
    ...
    FOR v_user IN 
      ...
      WHERE np.notification_type = 'attachment_uploaded'
        ...
        AND NEW.uploaded_by != u.user_id::text  -- Fixed: Compare TEXT to TEXT
        AND (
          NEW.uploaded_by != v_kpi.owner_user_id::text  -- Fixed: Compare TEXT to TEXT
...
COMMENT ON FUNCTION notify_kpi_attachment_upload() IS 'Fixed: uploaded_by is TEXT, cast UUIDs to TEXT for comparison';
COMMENT ON FUNCTION notify_evidence_attachment_upload() IS 'Fixed: uploaded_by is TEXT, cast UUIDs to TEXT for comparison';
