-- Fix role names in notification triggers
-- Role enum values are: 'admin', 'editor', 'viewer', 'auditor' (lowercase)
  ...
  IF TG_OP = 'INSERT' AND NEW.is_active = true THEN
    ...
    FOR v_user IN 
      ...
      WHERE np.notification_type = 'attachment_uploaded'
        ...
        AND u.user_id != NEW.uploaded_by
        AND (
          u.user_id = v_kpi.owner_user_id
          OR EXISTS (
            SELECT 1 FROM user_role ur
            INNER JOIN role r ON r.role_id = ur.role_id
            WHERE ur.user_id = u.user_id
            AND r.name = 'admin'  -- Fixed: lowercase 'admin'
          )
        )
    LOOP
...
COMMENT ON FUNCTION notify_kpi_attachment_upload() IS 'FINAL: Correct role name (lowercase admin), all UUID comparisons fixed';
COMMENT ON FUNCTION notify_evidence_attachment_upload() IS 'FINAL: Correct role name (lowercase admin), all UUID comparisons fixed';
