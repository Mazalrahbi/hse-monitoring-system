-- Fix type mismatch error in attachment notification triggers
  ...
  IF TG_OP = 'INSERT' AND NEW.is_active = true THEN
    ...
    FOR v_user IN 
      ...
      WHERE np.notification_type = 'attachment_uploaded'
        ...
        AND u.user_id::text != NEW.uploaded_by  -- Fixed: Cast user_id to text
        AND (
          v_kpi.owner_user_id::text = u.user_id::text  -- Fixed: Cast both sides
...
COMMENT ON FUNCTION notify_kpi_attachment_upload() IS 'Fixed type casting: Sends email notifications when attachments are uploaded to kpi_attachment (only to users with notifications ENABLED)';
COMMENT ON FUNCTION notify_evidence_attachment_upload() IS 'Fixed type casting: Sends email notifications when attachments are uploaded to evidence_attachment (only to users with notifications ENABLED)';
